...
function terrainHeight(xn: number, zn: number): number {
  // zn: -1 = north/mountains, +1 = south/sea
  const t = Math.max(0, (1 - zn) / 2); // 0 at sea, 1 at mountain
  ...
  const d =
    (Math.sin(xn * 8.7 + zn * 6.3) * 0.038 +
      Math.sin(xn * 3.2 - zn * 9.1) * 0.048 +
      Math.cos(xn * 12.1 + zn * 4.7) * 0.022) *
    t;

  return Math.max(0, r1 + r2 + r3 + d);
}
...
export default function TerrainRelief() {
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.FogExp2(0x12181f, 0.12);
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      const hv = terrainHeight(xn, zn);
      pos.setY(i, hv);

      const lh = hv / 1.35;
      if (lh < 0.04) {
        colors.push(0.18, 0.24, 0.32);
      } else if (lh < 0.25) {
        const f = (lh - 0.04) / 0.21;
        colors.push(0.3 + f * 0.12, 0.36 + f * 0.06, 0.42 + f * 0.04);
      } else if (lh < 0.65) {
        const f = (lh - 0.25) / 0.4;
        colors.push(0.42 + f * 0.14, 0.42 + f * 0.04, 0.46 + f * 0.04);
      } else {
        colors.push(0.62, 0.62, 0.65);
      }
    }
